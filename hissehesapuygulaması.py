...
if page == "TEMEL":
    ...
    if hisse_adi:
      ...
      potans=((ortalama_hesap - c3)*100)/c3
      ...
      st_echarts(option)
      bist = ['ASELS', 'ULKER', 'BOBET' ] 
      hisse_getirileri = []  
      if st.button('Verileri Çek ve Analiz Et'):
          for hisse_adi in bist:
                try:
                    # URL'yi oluşturun
                    url1 = "https://www.isyatirim.com.tr/tr-tr/analiz/hisse/Sayfalar/sirket-karti.aspx?hisse=" + hisse_adi
                    
                    # Web sitesinden veriyi çekin
                    r1 = requests.get(url1)
                    s1 = BeautifulSoup(r1.text, "html.parser")
                    
                    # Yıl ve dönem bilgilerini çekin
                    secim = s1.find("select", id="ddlMaliTabloFirst")
                    secim2 = s1.find("select", id="ddlMaliTabloGroup")
                    ...
                    if len(tarihler) >= 4:
                        ...
                        # Hesaplamalar yapın (örneğin özkaynaklar, ödenmiş sermaye vb.)
                        # Burada önceki kod parçanızdaki hesaplamaları yapabilirsiniz
                        
                        # Potansiyel getiriyi hesaplayın ve hisse adıyla birlikte listeye ekleyin
                        hisse_getirileri.append({
                            "Hisse Adı": hisse_adi,
                            "Potansiyel Getiri": potansiyel_getiri  # Burada potansiyel getiriyi hesaplayın
                        })
                        
                        # Kısa bir bekleme süresi ekleyin (bot olmaması için)
                        time.sleep(0.5)
